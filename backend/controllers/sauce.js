...
exports.modifySauce = (req, res, next) => {
  Sauce.findOne({
    ...
    if (sauce.userId === req.token.userId) {
      // Vérification de l'égalité entre le userId de la sauce et celui du token
    ...
    .then((sauce) => {
      switch (req.body.like) {
        case 0: // Si l'état du like est nul
          if (sauce.usersLiked.includes(userId))
            sauce.usersLiked = sauce.usersLiked.filter(
              (user) => user !== userId
            ); // Je vérifie et corrige le tableau en retirant le userId des likes
          if (sauce.usersDisliked.includes(userId))
            sauce.usersDisliked = sauce.usersDisliked.filter(
              (user) => user !== userId
            ); // Je vérifie et corrige le tableau en retirant le userId des dislikes
          break;
        ...
        case -1: // Si l'état du like est dislike
          ...
          if (!sauce.usersDisliked.includes(userId))
            sauce.usersDisliked.push(userId); // Je vérifie et corrige le tableau en ajoutant le userId dans les dislikes
          break;
        default:
          res.status(400).json({
            message: "Bad request",
